...
// Options controls process execution behavior for an envd instance.
type Options struct {
	// DisableBashLoginShell rewrites exact /bin/bash -l -c process requests as /bin/bash -c.
	DisableBashLoginShell bool
}

// Service implements the envd process RPC endpoints and tracks live processes.
type Service struct {
...
func Handle(
	...
	service := newService(l, defaults, cgroupManager, options)
