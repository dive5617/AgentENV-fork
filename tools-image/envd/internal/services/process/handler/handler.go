...
// New prepares a process handler with the resolved user, environment, working directory,
// resource policy, I/O streams, and optional command compatibility behavior.
func New(
	ctx context.Context,
	user *user.User,
	...
	command, args := commandForExecution(req.GetProcess(), disableBashLoginShell)
	...
	wrapperArgs := append([]string{"-c", oomWrapperScript, "--", command}, args...)
...
func commandForExecution(process *rpc.ProcessConfig, disableBashLoginShell bool) (string, []string) {
	command := process.GetCmd()
	args := process.GetArgs()

	if disableBashLoginShell &&
		command == "/bin/bash" &&
		len(args) == 3 &&
		args[0] == "-l" &&
		args[1] == "-c" {
		return command, []string{"-c", args[2]}
	}
