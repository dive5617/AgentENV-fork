...
func parseFlags() {
	...
	flag.BoolVar(
		&disableBashLoginShell,
		"disable-bash-login-shell",
		false,
		"execute /bin/bash -l -c process requests without the login-shell flag",
	)
...
// main initializes envd services and serves the guest control-plane APIs.
func main() {
	parseFlags()
	...
	processService := processRpc.Handle(m, &processLogger, defaults, cgroupManager, processRpc.Options{
		DisableBashLoginShell: disableBashLoginShell,
	})
